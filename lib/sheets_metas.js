...
class SheetsMetasService {
  ...
  async getSheetMetas() {
    ...
    if (!sheet) {
      ...
      sheet = await this.doc.addSheet({
        title: 'Metas',
        headerValues: ['ID', 'Usuario', 'Atividade', 'Tipo', 'MetaTotal', 'Realizado', 'Status', 'Descricao', 'DataCriacao']
      });
    }
    return sheet;
  }
  ...
  /**
   * Cria uma nova atividade/meta para um usuário existente na base.
   * tipo: 'hora' | 'numero' | 'valor'
   */
  async criarMeta(usuario, atividade, tipo, metaTotal, descricao) {
    try {
      if (!usuario) return { ok: false, msg: 'Usuário não informado' };
      ...
      console.log(`[METAS] ✓ Meta criada: ${usuario} - ${atividade} (${tipo}) - ${meta}`);
      return { ok: true, msg: 'Atividade adicionada com sucesso!', id };
    } catch (error) {
