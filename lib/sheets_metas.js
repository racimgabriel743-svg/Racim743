...
class SheetsMetasService {
  ...
  /**
   * Retorna o nome do cabeçalho da COLUNA A (primeira coluna) da aba informada.
   * Usado para ler os usuários independente de como a coluna A esteja nomeada.
   */
  async getNomeColunaA(sheet) {
    await sheet.loadHeaderRow();
    return sheet.headerValues[0];
  }

  /**
   * Confirma que o usuário existe na aba "Usuarios" (coluna A) da base (mesma usada no login).
   */
  async usuarioExiste(usuario) {
    await this.init();
    const sheet = this.doc.sheetsByTitle['Usuarios'];
    if (!sheet) return false;
    const colunaA = await this.getNomeColunaA(sheet);
    const rows = await sheet.getRows();
    return rows.some(r => String(r.get(colunaA) || '').trim() === String(usuario || '').trim());
  }

  /**
   * Lista todos os usuários existentes na COLUNA A da aba "Usuarios" (mesma base do login),
  ...
  async listarUsuarios() {
    try {
      ...
      const usuarios = [...new Set(
        rows.map(r => String(r.get(colunaA) || '').trim()).filter(Boolean)
